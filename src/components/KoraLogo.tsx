...
export const KoraWordmark = ({ className = "h-6", fill = "currentColor" }: { className?: string; fill?: string }) => (
  <svg
    xmlns="http://www.w3.org/2000/svg"
...
type KoraWordmarkAnimatedProps = {
  className?: string;
  fill?: string;
  /** Slightly tighter motion for compact loaders */
  compact?: boolean;
};

/**
 * Loading / brand animation: glyphs rise in left→right, a soft underline
 * sweeps under the mark, then a calm accent glow breathes behind it.
 */
export function KoraWordmarkAnimated({
  className = "h-11",
  fill = "currentColor",
  compact = false,
  ...
  return (
    <div className="relative inline-flex flex-col items-center">
      ...
      <div
        className="relative mt-2.5 w-[92%] overflow-hidden"
        style={{ height: compact ? 2 : 3 }}
        aria-hidden
      >
        <motion.div
          className="absolute inset-y-0 left-0 rounded-full bg-kindle-accent/75"
