...
function LetterWheel({
 ...
 exit={{opacity: 0}}
 className="absolute -top-10 left-1/2 -translate-x-1/2 z-10 px-3 py-1.5 rounded-full bg-kindle-accent text-kindle-bg font-serif font-bold tracking-[0.2em] text-sm shadow-lg"
 >
 {word}
 </motion.div>
 ) : null}
 ...
 x2={b.x}
 y2={b.y}
 stroke="var(--color-kindle-accent)"
 strokeWidth="4"
 strokeLinecap="round"
 />
 );
 ...
 cy={p.y}
 r="18"
 fill={active ?"var(--color-kindle-accent)":"var(--color-kindle-card)"}
 stroke={active ?"var(--color-kindle-accent)":"var(--color-kindle-border)"}
 strokeWidth="2"
 />
 <text
 x={p.x}
 ...
 fill={active ?"var(--color-kindle-bg)":"var(--color-kindle-text)"}
 fontSize="16"
 fontFamily="serif"
 fontWeight="700"
 >
...
export default function CrosswordGame({open, onClose, variant ="fullscreen", onOpenScores}: CrosswordGameProps) {
 ...
 >
 <motion.div
 className="relative w-full max-w-2xl h-[90vh] max-h-[92vh] bg-kindle-bg text-kindle-text border border-kindle-border rounded-3xl overflow-hidden flex flex-col shadow-2xl"
 ...
 >
 <CrosswordInner onClose={onClose} v={effectiveV} setV={setV} onOpenScores={onOpenScores} isMobile={isMobile} />
 </motion.div>
 </motion.div>
 </AnimatePresence>
 );
 ...
 <AnimatePresence>
 <motion.div
 className="fixed inset-0 z-[80] flex flex-col bg-kindle-bg text-kindle-text w-full h-full"
 initial={{opacity: 0}}
 animate={{opacity: 1}}
 exit={{opacity: 0}}
 role="dialog"
 ...
 function CrosswordInner({onClose, v, setV, onOpenScores, isMobile}: {onClose: () => void; v:"fullscreen"|"popup"; setV: (x:"fullscreen"|"popup") => void; onOpenScores?: () => void; isMobile?: boolean}) {
 return (
 <>
 <div
...
"radial-gradient(ellipse at 20% 0%, rgba(212,165,116,0.18), transparent 50%), radial-gradient(ellipse at 90% 100%, rgba(120,90,60,0.2), transparent 45%)",
 ...
 <header className="relative z-10 flex flex-col border-b border-kindle-border bg-kindle-card kora-safe-top">


 <div className="flex items-center justify-between gap-3 px-4 py-3">
 <div className="flex items-center gap-2.5 min-w-0">
 <div className="p-2 rounded-xl bg-kindle-accent/15 border border-kindle-accent/25">
 <Grid3X3 className="w-4 h-4 text-kindle-accent"/>
 </div>
 ...
 mode === m.id
 ?"border-kindle-accent/60 bg-kindle-accent/15"
 :"border-kindle-border bg-kindle-card ] hover:bg-kindle-card/80 :bg-white/[0.06]"
}`}
 >
 <p className="font-serif font-bold text-kindle-text">{m.title}</p>
 <p className="text-[10px] text-kindle-text-muted mt-0.5 leading-snug">{m.blurb}</p>
 </button>
 ))}
 </div>
 ...
 >
 <div>
 <p className="font-serif text-xl font-bold text-kindle-text">{DIFFICULTY_LABELS[diff].title}</p>
 <p className="text-[11px] text-kindle-text-muted mt-0.5">{DIFFICULTY_LABELS[diff].blurb}</p>
 <p className="text-[10px] font-mono text-kindle-text-muted/80 mt-1.5">
 Continue at level {bestLevel[mode][diff] || 1}
 </p>
 </div>
 <ChevronRight className="w-5 h-5 text-kindle-text-muted"/>
 </motion.button>
 ))}
 </div>
 ...
 className="w-full py-3 rounded-xl border border-kindle-border bg-kindle-card ] text-[11px] font-bold uppercase tracking-widest text-kindle-text hover:opacity-100 transition"
 >
 ...
 ?"bg-kindle-accent/25 text-kindle-accent"
 ...
 animate={{opacity: 1, y: 0}}
 exit={{opacity: 0, y: -4}}
 className="rounded-2xl border border-kindle-accent/40 bg-kindle-accent/15 px-4 py-3 text-kindle-text"
 >
 ...
 <div
 key={dir}
 className="rounded-2xl border border-kindle-border bg-kindle-card ] p-3 max-h-48 overflow-y-auto"
 >
 <h3 className="text-[10px] font-bold uppercase tracking-widest text-kindle-text-muted mb-2">
 {dir}
 </h3>
 ...
 activeWord?.id === w.id
 ?"bg-kindle-accent/25 text-kindle-text font-bold"
 :"opacity-70 hover:opacity-100 hover:bg-black/5 :bg-white/5"
}`}
 >
 ...
 {screen ==="play"&& puzzle ? (
 <footer className="relative z-10 border-t border-kindle-border px-4 py-3 pb-[max(0.75rem,var(--kora-safe-bottom))] bg-kindle-card/90 /90 backdrop-blur-md">
 <AnimatePresence>
 {statusMsg ? (
 <motion.p
 ...
 animate={{opacity: 1, y: 0}}
 exit={{opacity: 0}}
 className="text-center text-[11px] font-mono text-amber-700 mb-2"
 >
 {statusMsg}
 </motion.p>
 ) : null}
 ...
 type="button"
 onClick={revealCell}
 className={`flex-1 py-2.5 rounded-xl border border-kindle-border text-[10px] font-bold uppercase tracking-wider flex items-center justify-center gap-1.5 hover:bg-black/5 :bg-white/5 transition ${
 ...
 type="button"
 onClick={goNext}
 className="flex-[1.4] py-2.5 rounded-xl bg-kindle-accent text-kindle-bg text-[10px] font-bold uppercase tracking-wider flex items-center justify-center gap-1.5 hover:brightness-110 transition"
 >
 ...
 className="w-full max-w-sm rounded-3xl border border-kindle-accent/40 bg-kindle-card p-6 text-center shadow-2xl text-kindle-text"
 >
 ...
 className="flex-1 py-2.5 rounded-xl border border-kindle-border text-[10px] font-bold uppercase tracking-wider"
 >
 Menu
 </button>
 <button
 ...
 className="flex-1 py-2.5 rounded-xl bg-kindle-accent text-kindle-bg text-[10px] font-bold uppercase tracking-wider"
 >
 Level {level + 1}
 </button>
 </div>
