...
export default function WordSearchGame({open, onClose, variant ="fullscreen", onOpenScores}: WordSearchGameProps) {
 ...
 >
 <WordSearchInner onClose={onClose} v={effectiveV} setV={setV} onOpenScores={onOpenScores} isMobile={isMobile} />
 </motion.div>
 </motion.div>
 </AnimatePresence>
 );
 ...
 function WordSearchInner({onClose, v, setV, onOpenScores, isMobile}: {onClose: () => void; v:"fullscreen"|"popup"; setV: (x:"fullscreen"|"popup") => void; onOpenScores?: () => void; isMobile?: boolean}) {
 return (
 <>
 <div
...
"radial-gradient(ellipse at 70% 0%, rgba(212,165,116,0.16), transparent 50%), radial-gradient(ellipse at 10% 100%, rgba(90,120,100,0.18), transparent 45%)",
 ...
 <Search className="w-4 h-4 text-kindle-accent"/>
 </div>
 ...
 className="w-full text-left rounded-2xl border border-kindle-border bg-kindle-card ] hover:bg-kindle-card/80 :bg-white/[0.06] p-4 transition flex items-center justify-between gap-3"
 >
 <div>
 <p className="font-serif text-xl font-bold text-kindle-text">{WORD_SEARCH_LABELS[diff].title}</p>
 <p className="text-[11px] text-kindle-text-muted mt-0.5">{WORD_SEARCH_LABELS[diff].blurb}</p>
 ...
 animate={{opacity: 1}}
 exit={{opacity: 0}}
 className="text-center font-serif font-bold tracking-[0.25em] text-kindle-accent"
 >
 {selectingWord}
 </motion.p>
 ) : (
 ...
 <div
 ref={boardRef}
 className="mx-auto w-fit max-w-full touch-none select-none rounded-2xl border border-kindle-border bg-kindle-card /80 p-2 shadow-xl"
 onPointerDown={(e) => {
 ...
 : solved
 ?"bg-kindle-accent/35 text-kindle-text border-kindle-accent/50"
 ...
 found.has(w.word)
 ?"bg-kindle-accent/30 text-amber-800 line-through opacity-70"
 :"bg-black/5 border border-kindle-border opacity-80"
}`}
 >
 {w.word}
 </span>
 ...
 type="button"
 onClick={hint}
 className="flex-1 py-2.5 rounded-xl border border-kindle-border text-[10px] font-bold uppercase tracking-wider flex items-center justify-center gap-1.5 hover:bg-black/5 :bg-white/5"
 >
 ...
 type="button"
 onClick={goNext}
 className="flex-[1.4] py-2.5 rounded-xl bg-kindle-accent text-kindle-bg text-[10px] font-bold uppercase tracking-wider flex items-center justify-center gap-1.5 hover:brightness-110"
 >
