...
// "kora" wordmark glyph paths (shared with KoraLogo.KoraWordmark).
const KORA_PATHS = [
...
export default function KoraWordmarkReveal({
  ...
  return (
    <div ref={ref} className="flex flex-col items-center gap-4">
      <motion.div
        initial={{ scale: 1.15 }}
        animate={{ scale: filling ? 0.7 : 1.15 }}
        transition={{ duration: 1.4, ease: [0.45, 0.05, 0.55, 0.95] }}
        className="w-[min(80vw,420px)]"
      >
        <svg
          ...
          {KORA_PATHS.map((d, i) => (
            <motion.path
              ...
              transition={{
                ...
                fillOpacity: { duration: 0.9, ease: "easeOut", delay: 0.4 },
              }}
            />
          ))}
        </svg>
